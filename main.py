...
def send_welcome(message):
    ...
    bot.send_message(message.chat.id,
                     "Добро пожаловать, {0.first_name}!\nЯ - <b>{1.first_name}</b>, бот созданный чтобы разрешить "
                     "ваши жизненно важные споры.".format(
                         message.from_user, bot.get_me()),
                     parse_mode='html')
    bot.send_message(message.chat.id, 'Переключитесь на клавиатуру с кнопками)\n\n'
                                      'или просто напишите мне:\n\n'
                                      '" М " - если хотите подбросить монетку 🪙\n'
                                      '" К "- если хотите бросить кость 🎲 \n'
                                      '" Д " или " Н "- если хотите просто узнать: да 👍 или нет 👎',
...
def lalala(message):
    if message.text == '🪙 Подбросить монетку' or message.text == 'М' or message.text == 'м' or message.text == 'M':
    ...
    elif message.text == '🎲 Бросить кость' or message.text == 'K' or message.text == 'к' or message.text == 'К':
        bot.send_message(message.chat.id, 'Бросаем:', reply_markup=markupi)

    elif message.text == 'Да / Нет' or message.text == 'Н' or message.text == 'н' or message.text == 'H' or message.text == 'Д' or message.text == 'д':
        m = random.randint(0, 2)
        if m == 0:
        ...
        bot.send_message(message.chat.id, 'Переключитесь на клавиатуру с кнопками, пожалуйста)\n\n'
                                          'или просто напишите мне:\n\n'
                                          '" М "-если хотите подбросить монетку 🪙 \n'
                                          '" К "- если хотите бросить кость 🎲 \n'
                                          '" Д " или " Н "- если хотите просто узнать: да 👍 или нет 👎', reply_markup=markup2)
